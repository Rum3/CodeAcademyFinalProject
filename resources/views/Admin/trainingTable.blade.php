...
<div class="relative left-60 w-3/4 overflow-hidden rounded-lg border">
  <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
    ...
    <a href="{{ route('training.form') }}" class="flex items-center text-green-500 hover:text-green-700">
        ...
        <th scope="col" class="px-6 py-4 font-large text-gray-900">Trainings description</th>
    ...
    <tbody class="divide-y divide-gray-100 border-t border-gray-100">
      ...
      <tr class="hover:bg-gray-50">
        <td class="px-6 py-4">{{ $training->training_title }}</td>
        <td class="px-6 py-4">{{ $training->description }}</td>
        <td class="px-6 py-4">
          <div class="flex">
                <a href="{{ route('training.form', ['id' => $training->id]) }}" class="text-blue-500 mr-2">
